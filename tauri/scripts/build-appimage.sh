...
echo "==> Wiring in the WebKit exec-path fix (see issue #336)"
# AppRun itself is autogenerated fresh by linuxdeploy on every build (see
# its own "autogenerated" comment), so this line can't survive being
# hand-edited once -- inject it here, after each build, instead.
cp "$SCRIPT_DIR/webkit-execdir-bwrap.sh" "$APPDIR/apprun-hooks/webkit-execdir-bwrap.sh"
if ! grep -q "webkit-execdir-bwrap.sh" "$APPDIR/AppRun"; then
  sed -i '/linuxdeploy-plugin-gtk.sh/a\
source "$this_dir"/apprun-hooks/"webkit-execdir-bwrap.sh"' "$APPDIR/AppRun"
fi
