...
def create_recipe_item_route(
    recipe_item: RecipeItemCreate,
    ...
    """
    return create_recipe_item(db, recipe_item, organization_id)


@router.get("/{finished_product_id}", response_model=list[RecipeItemResponse])
def list_recipe_items_route(
    finished_product_id: int,
    db: Session = Depends(get_db),
    organization_id: int = Depends(get_current_org_id),
):
    """Lists every ingredient in a dish's recipe/BOM."""
    return list_recipe_items(db, finished_product_id, organization_id)
...
def update_recipe_item_route(
    ...
    return update_recipe_item_quantity(db, recipe_item_id, update.quantity, organization_id)


@router.delete("/{recipe_item_id}", status_code=204)
def delete_recipe_item_route(
    recipe_item_id: int,
    db: Session = Depends(get_db),
    organization_id: int = Depends(get_current_org_id),
):
